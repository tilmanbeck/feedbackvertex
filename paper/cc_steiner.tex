...
\begin{figure}
  \centering
    \includegraphics[width=0.5\textwidth]{./imgs/terminal_v1.png}
  	\caption{Konsistente Cuts}
	\label{fig:st_cut}
\end{figure}

In der Abbildung \ref{fig:st_cut} zeigt beispielhaft die Bildung von konsistenten Cuts. Die Knoten mit doppeltem Rand stellen Terminale da. Das Terminal $A$ wird als $v_1$ Terminal festgelegt. Zur vereinfachung wurde die Gewichtsfunktion $\omega$ weggelassen.
...
Ohne die Restriktion $v_1 \in X_1$ gäbe es die doppelte Menge an konsistenten Cuts. Bei jedem Cut könnte die Mengen $X_1$ und $X_2$ vertauscht werden. Durch die Restriktion von $v_1$ wird dies vermieden.
